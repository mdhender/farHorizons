...
import (
	"github.com/mdhender/farHorizons/internal/common"
	"log"
	"os"
)
...
var last_random uint64

/* This program will generate a completely new galaxy and write the results
   to files "galaxy.dat", "stars.dat" and "planets.dat". */

func main() {
	var d, i, n, galaxy_file, star_file, planet_file, using_defaults int
	var x, y, z, real_x, real_y, real_z, desired_num_stars, galactic_diameter, sq_distance_from_center int
	var st_index, pl_index, star_type, star_color, star_size, d_num_species, star_num_planets int
	var num_stars, galactic_radius, num_planets, max_planets, num_wormholes int
	var volume, l_radius, chance_of_star, num_bytes, star_data_size, planet_data_size, dx, dy, dz, distance_squared int
	var cp, star_here [MAX_DIAMETER][MAX_DIAMETER]*common.CString

	var current_star, star_base, star, worm_star *common.StarData
	var planet_base, planet *common.PlanetData
...
again3:
	...
	star_data_size = num_stars
	star_base = make([]*StarData, num_stars)

	max_planets = 9 * num_stars /* Maximum number possible. */
	planet_base = make([]*PlanetData, max_planets)
